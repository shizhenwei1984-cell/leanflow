...
Active runs persist state v7 and complete compact operation identity: explicit `currentBuildRound`, Gate/LSP/repair leases, approved validation contract and digest, finalized manifest, operational retry snapshot, and structured BLOCKED recovery state. Restore normalizes from `defaultState` and immediately persists any migration/reconciliation fingerprint change. A legacy Gate-ready or successful-finalizing state is accepted only when its v2 finalized snapshot and transactional nonce match the run, plan, contract, and round. If that authority still binds a v1/v2 BUILD record, restore first verifies the old checkpoint read-only, persists a non-authoritative evidence-recovery state, and only then rewrites the record to v3; this ordering is restart-safe. Re-finalization redispatches an interrupted Gate, while a previously settled PASS returns directly to `finalizing` without consuming another verdict. Missing authority becomes explicit re-finalization or `/flowcontinue` recovery, and an incomplete legacy operational retry clears lease/retry authority and pauses safely.

Each async BUILD action receives an immutable identity containing session ID, optional branch ID, run/plan/contract identity, BUILD round, activation epoch, and a unique operation ID. The BUILD record's baseline capture, validation append, and finalization transactions share an activation-keyed FIFO (`session/run/round/epoch`). Any activation transition clears pending observation correlation, aborts validation processes, and makes stale completions and staged writes discard themselves before publication.
