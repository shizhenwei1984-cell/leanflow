...
Treat the extension-generated build/evidence artifacts as provenance, not a substitute for it: Gate authority exists only when LeanFlow has finalized a nonce-bound v2 manifest from current BUILD record v3 observations. v1 observations and v2 observations lacking full operation provenance, or any partial or foreign run/round/plan/contract identity, are not validation evidence; report the resulting required-evidence gap as `BLOCKED` rather than inferring a PASS.
