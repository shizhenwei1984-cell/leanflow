...
Return exactly one JSON object matching the agent-owned strict schema. `PASS` has no blocking findings. `FAIL` has one or more blocking correctness, validation_failure, plan_deviation, missing_change, or regression_risk findings. Never make style or naming blocking.
