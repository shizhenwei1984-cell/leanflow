...
5. Implement the approved plan in Main; do not create an implementer, developer, coder, or builder subagent. The extension records LSP observations and every validation it executes from the immutable approved validation contract; blocked or skipped calls never become evidence.
6. For every approved validation ID listed by `leanflow_capture_baseline`, call `leanflow_run_validation({ validationId: "<validation-id>" })`. The extension executes the pre-approved executable/argv tuple directly with no shell, caller-controlled environment, cwd, timeout, or async options; it captures repository fingerprints immediately before and after, rejects repository-mutating validations, and records the result atomically.
7. Call `leanflow_finalize_artifacts({})`. The extension requires a current successful observation for every required validation ID, then revalidates plan/contract identity, baseline/final HEAD, final status, tracked binary diff, sorted untracked binary patches, repository fingerprint, and the 1 MiB per-artifact limit.
8. The finalizer mechanically writes and verifies `local://<slug>-build.md`, `local://<slug>-diff.md`, and `local://<slug>-evidence.md`, then atomically commits a digest-bound finalized Gate manifest. Main must never write or edit those files or internal records directly. Gate has no shell access; the generated evidence artifact is its only runtime evidence source.
...
- `FAIL`: consumes one of the two settled-verdict attempts in the current repair cycle. On the first `FAIL`, Main repairs, re-runs every required validation through `leanflow_run_validation`, calls `leanflow_finalize_artifacts({})`, then calls Gate once more. On the second `FAIL`, LeanFlow pauses in `awaiting_human`; do not create further agents. The user may run `/flowcontinue [note]` to begin a human repair cycle (the verdict budget resets) or `/flowfinishfailed` to mark the run failed and finalize it.
- `BLOCKED`: required evidence is insufficient, inconsistent, unreadable, or does not match the run. It does not consume a verdict attempt and returns LeanFlow to BUILD. Run only the affected approved validation IDs through `leanflow_run_validation`, then call `leanflow_finalize_artifacts({})`; source mutation is forbidden. A Gate redispatch without a newly passed affected ID whose normalized output digest or repository fingerprint differs from its BLOCKED boundary pauses in `awaiting_human` instead of looping; a mechanically identical rerun and unrelated Bash/LSP observations are not progress. Operational Gate errors retain the same finalized manifest and permit only read-only inspection plus Gate redispatch.
