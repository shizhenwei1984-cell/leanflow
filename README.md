...
The current persisted formats are state v9 and BUILD evidence v3. Every asynchronous control callback holds an immutable `ControlOperationIdentity`—operation ID, session ID, run ID, activation epoch, captured phase, plan digest, optional artifact identity, and creation time—registered behind its transport `toolCallId`. A completion may mutate state only while that exact authority remains current; activation replacement invalidates all pending control authority, aborts validation processes, and causes stale results or staged publications to be discarded. BUILD-record mutations use a FIFO keyed by session/run/round/activation epoch; repair-lease replay uses its own FIFO keyed by session/run/from-round/to-round/transaction ID, checking authority immediately after acquisition so a duplicate replay is idempotent without serializing independent runs.
